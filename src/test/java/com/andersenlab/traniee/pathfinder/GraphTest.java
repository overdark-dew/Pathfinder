...
public class GraphTest {
    ...
    public void testOnePointGraph() {
        ...
        assertNotNull("Не null", onepoint);
    }
    ...
    public void testFindShortWay() {
        ...
        Graph begin = new Graph.Builder(n).edge(0, 1, 1).edge(0, 2, 2).edge(1, 2, 5).edge(1, 0, 1).edge(2, 0, 2)
                .edge(2, 1, 5).build();
        Graph test = new Graph.Builder(n).shortWayGraph(begin).build();
        Graph expect = new Graph.Builder(n).edge(0, 1, 1).edge(0, 2, 2).edge(1, 2, 3).edge(1, 0, 1).edge(2, 0, 2)
                .edge(2, 1, 3).build();
        ...
        for (int i = 0; i < n; ++i) {
            ...
            for (int j = 0; j < n; ++j) {
                ...
                assertEquals("Элементы должны быть равны", expect, test);
            }
